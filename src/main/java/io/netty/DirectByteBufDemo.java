...
public class DirectByteBufDemo {
    ...
    public void apiTest() {
        ...
        System.out.println("1.ByteBuf中的内容为===============>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");
        ...
        System.out.println("2.ByteBuf中的内容为===============>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");
        ...
        System.out.println("3.ByteBuf中的内容为===============>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");

        // 4.将读取的内容丢弃
        buf.discardReadBytes();
        System.out.println("将读取的内容丢弃后ByteBuf为========>" + buf.toString());
        System.out.println("4.ByteBuf中的内容为===============>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");

        // 5.清空读写指针
        buf.clear();
        System.out.println("将读写指针清空后ByteBuf为==========>" + buf.toString());
        System.out.println("5.ByteBuf中的内容为===============>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");
        ...
        System.out.println("写入一段内容后ByteBuf为===========>" + buf.toString());
        System.out.println("6.ByteBuf中的内容为===============>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");

        // 7.将ByteBuf清零
        buf.setZero(0, buf.capacity());
        System.out.println("将内容清零后ByteBuf为==============>" + buf.toString());
        System.out.println("7.ByteBuf中的内容为================>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");
        ...
        System.out.println("写入一段内容后ByteBuf为===========>" + buf.toString());
        System.out.println("8.ByteBuf中的内容为===============>" + Arrays.toString(ByteBufUtil.getBytes(buf)) + "\n");
        //  随机访问索引 getByte
        //  顺序读 read*
        //  顺序写 write*
